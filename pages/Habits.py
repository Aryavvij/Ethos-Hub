...
db_data = fetch_query("SELECT habit_name, EXTRACT(DAY FROM log_date)::int, status FROM habits WHERE user_email=%s AND EXTRACT(MONTH FROM log_date)=%s AND EXTRACT(YEAR FROM log_date)=%s", (user, month_num, year))
...
# 3. DATA EDITOR FIX
# We force every column to be a CheckboxColumn to remove the red triangles
