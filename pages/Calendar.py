...
# --- CALENDAR GRID (Issue #7 Symmetry Fix) ---
...
for week in cal_matrix:
    ...
    for i, day in enumerate(week):
        if day != 0:
            with cols[i]:
                # Fix: Use a border container with a fixed internal height
                with st.container(border=True):
                    ...
                    # Scrollable div keeps the box from expanding
                    st.markdown('<div style="height:70px; overflow-y:auto; overflow-x:hidden;">', unsafe_allow_html=True)
                    for eid, desc in events:
                        st.markdown(f"""
                            <div style="background:#76b372; color:white; padding:2px 5px; border-radius:3px; margin-bottom:2px; font-size:10px; display:flex; justify-content:space-between;">
                                <span>{desc}</span>
                            </div>
